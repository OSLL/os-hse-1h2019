...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	// Fill this function i
        ...
        if (!(*pgtab & PTE_P)) {
                ...
                pp->pp_ref++;
                *pgtab = page2pa(pp) | PTE_P | PTE_W | PTE_U; 
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
        ...
        for (i = 0; i < size; i += PGSIZE) {
            pte_t *entry = pgdir_walk(pgdir, (void*) (va + i), true);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	// LAB 3: Your code here.
        ...
        for (void *curr_va = (void *)ROUNDDOWN(va, PGSIZE); curr_va < end; curr_va += PGSIZE) {
                pte_t *pte = pgdir_walk(env->env_pgdir, curr_va, false);
                if (!pte || (uintptr_t)curr_va > ULIM || ((*pte & perm) != perm)) {
                        if (curr_va < va)
                                user_mem_check_addr = (uintptr_t)va;
                        else
