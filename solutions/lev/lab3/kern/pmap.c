...
page_init(void)
{
	...
	size_t i;
        ...
        for (i; i < PGNUM(PADDR(boot_alloc(0))); i++) {
                pages[i].pp_ref = 1;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	size_t dir = PDX(va);
        ...
        if (!(pgdir[dir] & PTE_P)) {
                ...
                page->pp_ref++;
                tbl = page2kva(page);
                pgdir[dir] = page2pa(page) | PTE_P | PTE_W | PTE_U;
        } else
                tbl = KADDR(PTE_ADDR(pgdir[dir]));
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	for (size_t i = 0; i < size; i += PGSIZE) {
                pte_t* rec = pgdir_walk(pgdir, (void*) (va + i), 1);
                *rec = (pa + i) | perm | PTE_P;
        }
}
...
{
	pte_t* rec;
        struct PageInfo* page = page_lookup(pgdir, va, &rec);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	uintptr_t va_aligned = ROUNDDOWN((uintptr_t) va, PGSIZE);
        ...
        for (uintptr_t addr = va_aligned; addr < end; addr += PGSIZE) {
                ...
                if (addr >= ULIM || !pte || (*pte & (perm | PTE_P)) != (perm | PTE_P)) {
