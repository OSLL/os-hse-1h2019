...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    uintptr_t fr = ROUNDDOWN((uintptr_t)va, PGSIZE);
    uintptr_t to = ROUNDUP((uintptr_t)(va + len), PGSIZE);
    for (; fr < to; fr += PGSIZE) {
        if (fr >= ULIM) {
            user_mem_check_addr = (user_mem_check_addr, fr);
            return -E_FAULT;
        }
        pte_t *pte = pgdir_walk(env->env_pgdir, (void*) fr, 0);
        if (!pte || ((PTE_P | perm) & *pte) != (PTE_P | perm)) {
            if (user_mem_check_addr < fr) {
                user_mem_check_addr = fr;
            }
            return -E_FAULT;
        }
    }
