...
duppage(envid_t envid, unsigned pn)
{
	...
	if (uvpt[pn] & (PTE_W | PTE_COW)) {
		r = sys_page_map(thisenv->env_id, va, envid, va, PTE_P | PTE_U | PTE_COW);
		if (r < 0) {
			return r;
		}

		r = sys_page_map(thisenv->env_id, va, thisenv->env_id, va, PTE_P | PTE_U | PTE_COW);
		if (r < 0) {
			return r;
		}
	} else {
		r = sys_page_map(thisenv->env_id, va, envid, va, PTE_P | PTE_U);
		if (r < 0) {
			return r;
		}
	}
