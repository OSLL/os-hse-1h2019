...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	env->env_tf = *tf;
	env->env_tf.tf_eflags = tf->tf_eflags | FL_IF;
	env->env_tf.tf_ds = tf->tf_ds | 3;
	env->env_tf.tf_es = tf->tf_es | 3;
	env->env_tf.tf_ss = tf->tf_ss | 3;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (((perm | PTE_SYSCALL) != PTE_SYSCALL) || (perm & ~(PTE_U | PTE_P))) {
		-E_INVAL;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	pte_t *src_pte;
	struct PageInfo *pageInfo = page_lookup(srcenv->env_pgdir, srcva, &src_pte);
	if (!pageInfo) {
		return -E_INVAL;
	}
	...
	int result = page_insert(dstenv->env_pgdir, pageInfo, dstva, perm);
	if (result < 0) {
		return -E_NO_MEM;
	}
