...
sys_env_set_status(envid_t envid, int status)
{
	...
	int ret = envid2env(envid, &target_env, 1);
	if (ret < 0) {
		return ret;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	struct PageInfo *new_page = page_alloc(true);
	ret = page_insert(target_env->env_pgdir, new_page, va, perm);
	if (ret < 0) {
		page_free(new_page);
		return ret;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	pte_t *pte;
	struct PageInfo *the_page = page_lookup(src_env->env_pgdir, srcva, &pte);
	if (!the_page) {
		return -E_INVAL;
	}
	...
	ret = page_insert(dest_env->env_pgdir, the_page, dstva, perm);
	if (ret < 0) {
		page_free(the_page);
		return ret;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uintptr_t)srcva < UTOP && (uintptr_t)env->env_ipc_dstva < UTOP) {
		...
		pte_t *pte;
		struct PageInfo *the_page = page_lookup(curenv->env_pgdir, srcva, &pte);
		if (!the_page) {
			return -E_INVAL;
		}
		...
		ret = page_insert(env->env_pgdir, the_page, env->env_ipc_dstva, perm);
		if (ret < 0) {
			page_free(the_page);
			return ret;
		}
