...
duppage(envid_t envid, unsigned pn)
{
	...
	if ((!(uvpt[pn] & PTE_SHARE)) && (uvpt[pn] & (PTE_W | PTE_COW))) {
		if (sys_page_map(0, va, envid, va, PTE_U | PTE_COW | PTE_P | (uvpt[pn] & PTE_AVAIL)) < 0) {
			panic("duppage : sys_page_map error");
		}
		if ((sys_page_map(0, va, 0, va, PTE_U | PTE_COW | PTE_P | (uvpt[pn] & PTE_AVAIL)) < 0)) {
			panic("duppage : sys_page_map error");
		}
	} else {
